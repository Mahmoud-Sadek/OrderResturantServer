...
import com.example.mahmoudsadek.orderresturantserver.common.DirectionJSONParser;
        ...
        if (ActivityCompat.checkSelfPermission(this, Manifest.permission.ACCESS_FINE_LOCATION) != PackageManager.PERMISSION_GRANTED
            ...
            if (mLastLocation != null) {
                ...
                if (currentMarker == null) {
                    currentMarker = mMap.addMarker(new MarkerOptions().position(yourLocation)
                            .title("Your Location")
                            .icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_RED)));

                } else {
                    currentMarker.setPosition(yourLocation);
                }
                mMap.moveCamera(CameraUpdateFactory.newLatLng(yourLocation));
                mMap.animateCamera(CameraUpdateFactory.zoomTo(17.0f));

                // after Add marker for  your Loaction , Add Marker for this order
                if (polyline == null)
                    drawRoute(yourLocation, Common.currentRequest);
            } else {
...
//            bitmap = Common.scaleBitmap(bitmap, 70, 70);
            ...
            MarkerOptions marker = new MarkerOptions().icon(BitmapDescriptorFactory.defaultMarker(BitmapDescriptorFactory.HUE_YELLOW))
            ...
            mService.getGeoCode(request.getAddress(), "AIzaSyDaRn9gxgYgtA4tOmduQcAoaNtiL7Dm3Pw").enqueue(new Callback<String>() {
                @Override
                public void onResponse(Call<String> call, Response<String> response) {
                    try {
    ...
    private void drawRoute(final LatLng yourLocation, String address) {
        mService.getGeoCode(address, "AIzaSyBM_sOa95aXILM6jo0tpqpp1hRA0l5D0xw").enqueue(new Callback<String>() {
            @Override
            public void onResponse(Call<String> call, Response<String> response) {
                try {
                    JSONObject jsonObject = new JSONObject(response.body().toString());
                    ...
                    Bitmap bitmap = BitmapFactory.decodeResource(getResources(), R.drawable.logo_eat_it);
                    bitmap = Common.scaleBitmap(bitmap, 70, 70);
                    ...
                    mService.getDirections(yourLocation.latitude + "," + yourLocation.longitude,
                            orderLocation.latitude + "," + orderLocation.longitude,
                            "AIzaSyBM_sOa95aXILM6jo0tpqpp1hRA0l5D0xw")
                            .enqueue(new Callback<String>() {
                                @Override
                                public void onResponse(Call<String> call, Response<String> response) {
                                    new ParserTask().execute(response.body().toString());
    ...
    private boolean checkPlayServises() {
        int rseultCode = GooglePlayServicesUtil.isGooglePlayServicesAvailable(this);
        if (rseultCode != ConnectionResult.SUCCESS) {
            if (GooglePlayServicesUtil.isUserRecoverableError(rseultCode)) {
                GooglePlayServicesUtil.getErrorDialog(rseultCode, this, PLAY_SERVICES_RESOLUTION_REQUEST);
            } else {
